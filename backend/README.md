...
A Rust-based backend for the XOXOPAY payment system, built on Stellar network with Soroban smart contracts.

## Architecture

The XOXOPAY backend provides the following services:
...
3. Environment variables with `XOXOPAY_` prefix
...
Direct transfers between two XOXOPAY users are exposed via the **Transfers** API. These endpoints construct an **unsigned Stellar transaction XDR** that the client signs and submits, keeping funds non-custodial.
...
- Create an **unsigned Stellar transaction XDR** representing a transfer from the authenticated user (`from_user_id`) to another XOXOPAY user (`to_user_id`).
...
- **`to_user_id`**: Target XOXOPAY user identifier. Must correspond to an existing row in the `users` table.
...
- **`to_user_id`**: Recipient XOXOPAY user ID.
...
```bash
cargo build --release
./target/release/XOXOPAY-backend
```
...
- Database pool size is configurable with `XOXOPAY_DATABASE__MAX_POOL_SIZE`, and query/index optimization lives in the migration set.
...
This project is part of the XOXOPAY ecosystem.

...
- Run with `LOG_FORMAT=json` and `RUST_LOG=xoxopay_backend=info,tower_http=info`.
- Ship stdout to Elasticsearch, Loki, Datadog, or another JSON log backend.
- Retain application logs for `XOXOPAY_OBSERVABILITY__LOG_RETENTION_DAYS` days, defaulting to 30.

Recommended error tracking:

- Configure `XOXOPAY_OBSERVABILITY__SENTRY_DSN` in production.

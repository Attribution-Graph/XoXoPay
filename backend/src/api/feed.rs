    ...
    PgPool: axum::extract::FromRef<S>,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let (username, address) = if token == "mock-jwt-token-string" {
            ("ebube.xoxopay".to_string(), "GABC1234EXAMPLESTELLARADDRESSXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX".to_string())
        } else {
            // Attempt to decode as JWT
            let secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| "xoxopay-jwt-secret-placeholder-very-long-key".into());

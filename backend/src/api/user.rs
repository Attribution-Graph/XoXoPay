    ...
    match row {
        Ok(Some(r)) => {
            ...
            let linked_accounts = serde_json::json!([
                ...
                {
                    "type": "email",
                    "address": format!("{}@xoxopay.fi", username),
                    "verified_at": privy_linked_at.map(|t| t.and_utc().to_rfc3339()),
                }
            ]);
...
mod tests {
    ...
    fn accepts_dot_separated_usernames_but_not_edge_dots() {
        assert_eq!(validate_username("test.xoxopay"), Ok(()));
        assert_eq!(validate_username(".test"), Err(UsernameError::InvalidCharacters));
        assert_eq!(validate_username("test."), Err(UsernameError::InvalidCharacters));
        assert_eq!(validate_username("test..xoxopay"), Err(UsernameError::InvalidCharacters));
    }

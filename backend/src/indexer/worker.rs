...
use super::parser::{parse_xoxopay_event, XoXoPayEvent, TokenSalvagedEvent, UserRegisteredEvent};
...
pub async fn process_event_batch_with_guard(
    ...
    let process_res: Result<(), Box<dyn Error + Send + Sync>> = async {
        for event in events {
            ...
            match parse_xoxopay_event(&topic, event) {
                XoXoPayEvent::YieldDeposited(e) => {

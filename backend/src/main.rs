...
use xoxopay_backend::api;
use xoxopay_backend::config;
use xoxopay_backend::db;
use xoxopay_backend::indexer;
use xoxopay_backend::services;
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "xoxopay-backend=debug,tower_http=debug".into()),
        )
        .with(tracing_subscriber::fmt::layer().json())
        .init();

    tracing::info!("Initializing XoXoPay Social Backend...");
...
async fn app_config() -> Json<AppConfigResponse> {
    Json(AppConfigResponse {
        ...
        ios_store_url: std::env::var("IOS_STORE_URL")
            .unwrap_or_else(|_| "https://apps.apple.com/app/xoxopay".into()),
        android_store_url: std::env::var("ANDROID_STORE_URL")
            .unwrap_or_else(|_| "https://play.google.com/store/apps/details?id=app.xoxopay".into()),
    })
}

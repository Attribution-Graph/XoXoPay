...
pub struct TelemetryConfig {
    ...
    /// Log level filter (e.g., "info", "debug", "xoxopay_backend=debug,tower_http=info")
...
pub fn init_tracing_with_config(config: TelemetryConfig) -> Result<(), Box<dyn std::error::Error>> {
    let env_filter = match config.log_filter {
        ...
        None => EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| EnvFilter::new("xoxopay_backend=info,tower_http=info")),
    };

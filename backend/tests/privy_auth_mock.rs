...
async fn setup_test_pool() -> PgPool {
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://postgres:postgres@localhost/xoxopay_test".to_string());
...
async fn create_test_app(pool: PgPool) -> Router {
    ...
    Router::new()
        .route(
            "/api/auth/challenge",
            get(xoxopay_backend::api::auth::get_challenge),
        )
        .route(
            "/api/auth/verify",
            post(xoxopay_backend::api::auth::verify_signature),
        )
        .route(
            "/api/auth/privy",
            post(xoxopay_backend::api::auth::privy_auth),
        )
        .with_state(xoxopay_backend::api::auth::AuthState {
            pool,
            privy: std::sync::Arc::new(xoxopay_backend::api::privy_jwks::PrivyJwksClient::new(
                "http://127.0.0.1:1/jwks".to_string(),
            )),
            privy_app_id: "test".to_string(),
        })
}
...
mod privy_auth_integration_tests {
    ...
    async fn test_privy_auth_rejects_address_linked_to_different_did() {
        ...
        let resp_1 = create_test_app(pool.clone())
            .await
            .oneshot(req_1)
            .await
            .unwrap();

...
async fn create_test_app(pool: PgPool) -> Router {
    ...
    Router::new()
        .route("/api/auth/challenge", get(xoxopay_backend::api::auth::get_challenge))
        .route("/api/auth/verify", post(xoxopay_backend::api::auth::verify_signature))
        .route("/api/auth/privy", post(xoxopay_backend::api::auth::privy_auth))
        .with_state(pool)
}

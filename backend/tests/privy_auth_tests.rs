...
use uuid::Uuid;
use xoxopay_backend::api::auth::AuthState;
use xoxopay_backend::api::privy_jwks::PrivyJwksClient;
...
fn create_test_app(pool: PgPool) -> Router {
    ...
    Router::new().nest(
        "/api/auth",
        xoxopay_backend::api::auth_routes_with_state(state),
    )
}

async fn protected_identity(
    Extension(user): Extension<xoxopay_backend::api::AuthenticatedUser>,
) -> String {
    user.address
}

fn create_protected_test_app(pool: PgPool) -> Router {
    xoxopay_backend::api::protected_routes(
        Router::new().route("/protected", get(protected_identity)),
        pool,
        xoxopay_backend::api::AuthTokenCache::new(),
    )
}

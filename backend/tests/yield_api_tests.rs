...
//! that has the XoXoPay schema applied.
...
async fn test_pool() -> PgPool {
    ...
    xoxopay_backend::db::run_migrations(&pool)
        .await
...
fn yield_router(pool: PgPool) -> Router {
    ...
    xoxopay_backend::api::yield_routes(pool)
}
...
async fn test_yield_service_daily_snapshots() {
    use xoxopay_backend::services::yield_service::YieldService;

 ...
 * Mock strategy
 * -------------
 * Specs clear cookies to act as a guest, or seed a `token` / `xoxopay-auth`
...
async function seedAuthCookies(page: Page): Promise<void> {
  await page.context().addCookies([
    AUTH_COOKIE,
    { ...AUTH_COOKIE, name: "xoxopay-auth", value: "1" },
  ]);
}
...
test.describe("Auth guard: unauthenticated visitors", () => {
  ...
  test("keeps the landing page reachable", async ({ page }) => {
    ...
    await expect(page.getByText("XoXoPay Merchant")).toBeVisible({
      timeout: 15_000,
    });
 ...
 * `loadFreighterApi()`, which prefers `window.__xoxopayFreighterApiMock__` outside
 ...
 */
const FREIGHTER_MOCK_KEY = "__xoxopayFreighterApiMock__";

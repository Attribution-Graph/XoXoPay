...
describe("Avatar", () => {
  ...
  it("renders image when uri is provided", () => {
    const { getByLabelText } = render(
      <Avatar uri="https://example.com/avatar.jpg" name="tolu.xoxopay" />
    );
    expect(getByLabelText("Avatar for tolu.xoxopay")).toBeTruthy();
  });

  it("renders initials fallback with accessibility label", () => {
    const { getByLabelText } = render(<Avatar name="tolu.xoxopay" />);
    expect(getByLabelText("Avatar placeholder for tolu.xoxopay")).toBeTruthy();
  });
});

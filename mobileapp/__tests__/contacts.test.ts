...
describe("batchMatchContacts", () => {
  ...
  it("sends hashes to the backend and returns matches", async () => {
    ...
    expect(result[0].username).toBe("tolu.xoxopay");
  ...
  it("maps backend matches back to contact names", async () => {
    ...
    mockFetch.mockImplementationOnce(async (url: string, init: { body: string }) => {
      ...
      return {
        ...
        json: async () => ({
          matches: [
            { phone_hash: hash1, username: "tolu.xoxopay", address: "GABCD" },
            { phone_hash: hash2, username: "kemi.xoxopay", address: "GXYZW" },
          ],
        }),
      };
    });
    ...
    expect(usernames).toContain("tolu.xoxopay");
    expect(usernames).toContain("kemi.xoxopay");
  });
  ...
  it("returns only matched contacts (not all contacts)", async () => {
    ...
    mockFetch.mockImplementationOnce(async (url: string, init: { body: string }) => {
      ...
      return {
        ok: true,
        json: async () => ({
          matches: [{ phone_hash: hash1, username: "tolu.xoxopay", address: "GABCD" }],
        }),
      };
    });

...
function makeItem(overrides: Partial<QueuedTransaction> = {}): QueuedTransaction {
  return {
    id: "test-id",
    endpoint: "https://api.xoxopay.app/api/v1/transfer",
    method: "POST",
    body: { recipient: "tolu.xoxopay", amount: "500" },
...
describe("enqueueTransaction", () => {
  it("appends a new item to the queue", async () => {
    ...
    expect(q[0].endpoint).toBe("https://api.xoxopay.app/api/v1/transfer");
...
describe("flushQueue", () => {
  ...
  it("processes multiple queued items sequentially", async () => {
    await writeQueue([
      makeItem({ id: "t1", endpoint: "https://api.xoxopay.app/a" }),
      makeItem({ id: "t2", endpoint: "https://api.xoxopay.app/b" }),
    ]);
    mockFetch

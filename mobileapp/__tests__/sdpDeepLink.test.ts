...
describe('parseSdpDeepLink', () => {
  ...
  it('parses a standard SDP deep link with token and amount', () => {
    const result = parseSdpDeepLink(
      'xoxopay://sdp/pay?token=abc123&amount=100',
    );
  ...
  it('parses a deep link with a decimal amount', () => {
    const result = parseSdpDeepLink(
      'xoxopay://sdp/pay?token=tok_99&amount=49.99',
    );
  ...
  it('parses a deep link with extra query parameters (ignores them)', () => {
    const result = parseSdpDeepLink(
      'xoxopay://sdp/pay?token=xyz&amount=250&ref=123&source=qr',
    );
  ...
  it('parses an HTTPS URL variant', () => {
    const result = parseSdpDeepLink(
      'https://xoxopay.app/sdp/pay?token=secure_token_1&amount=1000',
    );
  ...
  it('handles URL-encoded token values', () => {
    const result = parseSdpDeepLink(
      'xoxopay://sdp/pay?token=hello%20world&amount=55',
    );
  ...
  it('handles leading/trailing whitespace in the URL', () => {
    const result = parseSdpDeepLink(
      '  xoxopay://sdp/pay?token=trim_me&amount=10  ',
    );
  ...
  it('returns amount as number, not string', () => {
    const result = parseSdpDeepLink(
      'xoxopay://sdp/pay?token=num&amount=777',
    );
  ...
  it('returns null for a URL without query parameters', () => {
    expect(parseSdpDeepLink('xoxopay://sdp/pay')).toBeNull();
  });

  it('returns null when the token parameter is missing', () => {
    expect(parseSdpDeepLink('xoxopay://sdp/pay?amount=100')).toBeNull();
  });

  it('returns null when the amount parameter is missing', () => {
    expect(parseSdpDeepLink('xoxopay://sdp/pay?token=abc')).toBeNull();
  });

  it('returns null when token is present but empty', () => {
    expect(
      parseSdpDeepLink('xoxopay://sdp/pay?token=&amount=100'),
    ).toBeNull();
  });

  it('returns null when amount is present but empty', () => {
    expect(
      parseSdpDeepLink('xoxopay://sdp/pay?token=abc&amount='),
    ).toBeNull();
  });

  it('returns null when amount is not a number (NaN)', () => {
    expect(
      parseSdpDeepLink('xoxopay://sdp/pay?token=abc&amount=notanumber'),
    ).toBeNull();
  });

  it('returns null when amount is zero', () => {
    expect(
      parseSdpDeepLink('xoxopay://sdp/pay?token=abc&amount=0'),
    ).toBeNull();
  });

  it('returns null when amount is negative', () => {
    expect(
      parseSdpDeepLink('xoxopay://sdp/pay?token=abc&amount=-50'),
    ).toBeNull();
  });

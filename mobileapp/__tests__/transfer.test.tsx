...
// Mock SVG imports
jest.mock("../assets/icon-4.svg", () => "XoXoPayIcon");
...
async function advanceToStep1(renderResult: ReturnType<typeof render>) {
  ...
  // Step 0 shows two AccountTypeCards; the "XoXoPay User" one is already selected
  // by default (transferType starts as "XOXOPAY"). Press "Continue".
...
async function typeRecipient(renderResult: ReturnType<typeof render>, username: string) {
  ...
  const input = getByPlaceholderText(/Recipient XOXOPAY ID/i);
  await act(async () => {
...
describe("TransferScreen – Registry Warning Banners", () => {
  ...
  it("shows 'not registered' warning when resolve returns 404", async () => {
    ...
    expect(
      renderResult.getByText(/unknownuser is not registered on XoXoPay/)
    ).toBeTruthy();
  });
  ...
  it("shows 'registered' confirmation when resolve returns 200 OK", async () => {
    ...
    expect(
      renderResult.getByText(/existinguser is a registered XoXoPay user/)
    ).toBeTruthy();
  });
  ...
  it("shows checking indicator while resolve request is in flight", async () => {
    ...
    const input = renderResult.getByPlaceholderText(/Recipient XOXOPAY ID/i);
  ...
  it("does not show any warning for external wallet mode", async () => {
    ...
    // The warning banner condition requires transferType === "XOXOPAY",

...
export default function ReceiveScreen() {
  ...
  const [receiveType, setReceiveType] = useState<"XOXOPAY" | "external" | null>(
    null
  );

  const blinkId = "ejembiii.xoxopay";
  ...
  const qrValue = receiveType === "XOXOPAY" ? blinkId : sep0007Uri;
  ...
  const handleShare = async () => {
    try {
      const shareValue = receiveType === "XOXOPAY" ? blinkId : walletAddress;
  ...
  const handleCopy = () => {
    const copyValue = receiveType === "XOXOPAY" ? blinkId : walletAddress;
  ...
  const renderStep0 = () => (
    <View style={styles.stepContainer}>
      ...
      <View style={styles.cardsContainer}>
        <AccountTypeCard
          title="XoXoPay User"
          description="Receive instantly from any XoXoPay user via your XoXoPay ID"
          Icon={XoXoPayIcon}
          selected={receiveType === "XOXOPAY"}
          onPress={() => setReceiveType("XOXOPAY")}
        />
        <AccountTypeCard
      ...
      <View style={styles.qrContainer}>
        ...
        <View style={styles.idDisplaySection}>
          <View style={styles.idBadge}>
            <XoXoPayIcon width={18} height={18} />
          </View>
          <View style={styles.idTextContainer}>
            <Text style={styles.idLabel}>
              {receiveType === "XOXOPAY" ? "XoXoPay ID" : "Wallet Address"}
            </Text>
            <Text style={styles.idValue}>
              {receiveType === "XOXOPAY" ? blinkId : walletAddress}
            </Text>
          </View>
        </View>

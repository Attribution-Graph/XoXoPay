...
import type { XoXoPayUser } from "../src/types/user";
...
import XoXoPayIcon from "../assets/icon-4.svg";
...
function TransferScreen() {
  ...
  const [transferType, setTransferType] = useState<"XOXOPAY" | "external" | "BATCH" | null>(
    "XOXOPAY"
  );
  ...
  const [searchResults, setSearchResults] = useState<XoXoPayUser[]>([]);
  const [recentRecipients, setRecentRecipients] = useState<string[]>([]);
  const [searching, setSearching] = useState(false);
  const [showDropdown, setShowDropdown] = useState(false);
  const [selectedUser, setSelectedUser] = useState<XoXoPayUser | null>(null);
  ...
  const searchUsers = useCallback(async (query: string) => {
    ...
    try {
      ...
      const data: XoXoPayUser[] = await res.json();
  ...
  // Debounce search while user types (XOXOPAY mode only) + registry check + federation
  useEffect(() => {
    if (transferType !== "XOXOPAY") return;
  ...
  const handleSelectUser = useCallback((user: XoXoPayUser) => {
  ...
  // #699 — Load matched contacts on first XOXOPAY-mode step (lazy load, once)
  ...
  // Trigger contact load when user navigates to XOXOPAY recipient step.
  useEffect(() => {
    if (transferType === "XOXOPAY" && step === 1) {
      loadContacts();
    }
  ...
  const renderStep0 = () => (
    <View style={styles.stepContainer}>
      ...
      <View style={styles.cardsContainer}>
        <AccountTypeCard
          title="XoXoPay User"
          description="Send instantly to any XoXoPay user via their XOXOPAY ID"
          Icon={XoXoPayIcon}
          selected={transferType === "XOXOPAY"}
          onPress={() => setTransferType("XOXOPAY")}
        />
        <AccountTypeCard
  ...
  const renderStep1 = () => (
    <View style={styles.stepContainer}>
      <View style={styles.inputsSection}>
        {/* Recipient input + live-search dropdown (XOXOPAY mode only) */}
        <View style={styles.recipientSearchWrapper}>
          <Input
            placeholder={
              transferType === "XOXOPAY"
                ? "Recipient XOXOPAY ID (e.g. tolu.xoxopay)"
                : "Wallet Address"
            }
            value={recipient}
          ...
          />
          {/* Searching indicator */}
          {transferType === "XOXOPAY" && searching && (
            <ActivityIndicator
              size="small"
              color="#1A4B4A"
          ...
          {transferType === "XOXOPAY" && federationStatus === "resolving" && (
          ...
          {transferType === "XOXOPAY" && federationStatus === "resolved" && (
            <View
          ...
          {transferType === "XOXOPAY" && federationStatus === "error" && (
            <View
          ...
          (loadingContacts || matchedContacts.length > 0) && (
            ...
            >
              <View style={styles.contactsSectionHeader}>
                ...
                <Text style={styles.dropdownHeader}>Phone contacts on XoXoPay</Text>
                {loadingContacts && (
                  <ActivityIndicator
                    size="small"
        ...
        {/* ── Registry status warning banners (XOXOPAY mode only) ──────── */}
        {transferType === "XOXOPAY" && recipient.length >= 3 && (
          <>
            ...
            {usernameStatus === "not_registered" && (
              ...
              >
                <Ionicons name="alert-circle" size={20} color="#DC2626" />
                <Text style={styles.warningBannerTextDanger}>
                  @{recipient} is not registered on XoXoPay.
                </Text>
              </View>
            )}
            ...
            {usernameStatus === "registered" && (
              ...
              >
                <Ionicons name="checkmark-circle" size={20} color="#16A34A" />
                <Text style={styles.warningBannerTextSuccess}>
                  @{recipient} is a registered XoXoPay user.
                </Text>
              </View>
            )}
          </>
      ...
      <View style={styles.visibilitySection}>
        ...
        <Text style={styles.visibilityDesc}>
          {visibility === "PUBLIC" && "Visible to anyone on the XoXoPay network."}
      ...
      <View style={styles.summaryCardLarge}>
        ...
        <View style={styles.infoRow}>
          <View style={styles.recipientBadge}>
            <XoXoPayIcon width={16} height={16} />
          </View>

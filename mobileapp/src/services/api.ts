 ...
 * api.ts
 *
 * Central API client for the XoXoPay mobile app.
 *
 ...
 *  - `sendTransactionWithOfflineSupport` wraps any outgoing XoXoPay transaction
...
const API_BASE = process.env.EXPO_PUBLIC_API_URL ?? "https://api.xoxopay.app";
...
/**
 * Resolves a username and returns the result in XoXoPayUser format for compatibility
 * with existing UI components that expect the full user interface.
 * 
 * @param username - The username to resolve
 * @returns Promise resolving to a partial XoXoPayUser with username and address
 * @throws UsernameLookupError with specific error codes for different failure scenarios
 */
export async function resolveUsernameAsXoXoPayUser(username: string): Promise<{
  username: string;
  address: string;
  avatar_url: null;
  isVerified?: boolean;
  ...
  return {
    ...
    isVerified: false, // placeholder as noted in XoXoPayUser interface
  };
}
...
/**
 * Send a XoXoPay transaction request with offline-queue fallback (#687).
 *
 ...
 *   method: "POST",
 *   body: { recipient: "tolu.xoxopay", amount: "500", currency: "USDC" },
 * });
 *
 * if (queued) {
...
export async function recoverWalletViaPrivy(
  ...
  if (didRes.status === 404) {
    throw new Error(
      "No XoXoPay wallet is linked to this social identity. Please create an account first."
    );
  }

...
const NOTIFICATION_PREFERENCE_KEY = "xoxopay_notifications_enabled";
const PUSH_TOKEN_KEY = "xoxopay_push_token";
const NOTIFICATION_PROMPT_DISMISSED_KEY = "xoxopay_notifications_prompt_dismissed";
  ...
  try {
    ...
    const response = await apiFetch(`${API_BASE}/api/notifications/register`, {
      ...
      body: JSON.stringify({
        ...
        appId: Constants.manifest?.slug || Constants.expoConfig?.slug || "XOXOPAY",
      }),
    });
    return response.ok;
  } catch (error) {

...
# Setup directories
BASE_DIR = "/Users/mac/Desktop/codes/drips_network/xoxopay"
...
for i in range(1, 16):
    num = f"{i:03d}"
    if i == 1:
        title = "Implement Username Registration mapping Address -> XoXoPay ID"
        desc = "Implement the core registration function to allow users to register a unique XoXoPay username (e.g. ebube.xoxopay)."
        ...
        title = "Add validation rules for XoXoPay ID registration"
...
for i in range(1, 21):
    ...
    if i == 1:
        ...
        desc = "Support searching other users by their unique XoXoPay ID (alphanumeric search) for payment routing."
        files = "- `backend/src/api/user.rs`"
...
for i in range(1, 16):
    ...
    if i == 1:
        ...
        desc = "Allow searching users by XoXoPay ID when making a transfer."
        files = "- `mobileapp/app/transfer.tsx`"
        crit = "- Display matching users in list as user types XoXoPay ID."
